...
1. Mở `dist\RTZ-to-CSV.exe` / Open `dist\RTZ-to-CSV.exe`.
...
The public source repository is [github.com/ilohlove/rtz-converter](https://github.com/ilohlove/rtz-converter). Releases use strict `vMAJOR.MINOR.PATCH` tags and stable, immutable GitHub Releases only. The app checks the latest release at every opening and from **Cập nhật / Update**. It shows release notes and asks before downloading.

The updater downloads `RTZ-to-CSV.exe` and its `RTZ-to-CSV.exe.sha256` manifest over HTTPS, verifies the GitHub asset digest and SHA-256, then stages the file and restarts the application. If the installed directory is not writable, it opens the Release page instead. No telemetry is collected.
...
```powershell
gh api --method PUT repos/ilohlove/rtz-converter/immutable-releases `
...
The script runs tests, embeds the version from `app_metadata.py`, writes `dist\RTZ-to-CSV.exe`, and creates `dist\RTZ-to-CSV.exe.sha256`.
...
3. GitHub Actions verifies the tag, runs tests, builds the x64 GUI executable, and publishes the two release assets.

The initial release is `v1.0.0`. The tool only extracts route data; always validate the route separately in ECDIS.
